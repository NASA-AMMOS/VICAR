 ...
 *	21-dec-93	PR2532: Solaris prefers the termios interface to
 *			the terminal. The previous termio (ioctl) interface
 *			for SYSV systems did not work. TM would only get
 *			characters when a "\n" was typed...krw
 *	13-jun-94	PR2683: SCO needed the same fix as PR2532 above...krw
 *	04-aug-94	IRIX 5.2 port: needed the same fix as PR2532 above...dag
 */
...
/* This is defined in term.h, and conflicts with other definition */
#ifdef TERMINAL
#undef TERMINAL
#endif
#include <term.h>
...
FUNCTION CODE t_pos (FUNINT line, FUNINT column);
FUNCTION static VOID read_terminal 
(
...
#if (defined(sco) || (defined(sun) && OSMajorVersion >= 5)) || defined(irix5) || defined(__linux) || defined(linux) || defined(__APPLE__)
#define USE_CURSES 1
#endif
    ...
    static TEXT		control[250];	/* control strings
					   */
#ifdef USE_CURSES
    static struct termios init_char;  	/* initial characteristics	*/
    static struct termios single_char;  /* single-character mode        */
#else
 ...
 */
    ...
    FUNCTION VOID t_signal (int UNUSED(x))
    {
 ...
 */
    FUNCTION static int t_outc(int chr)
    {
    putc(chr, stdout);
    return OK;
    }
...
#ifdef USE_CURSES
    tcsetattr (0, TCSANOW, &single_char);	/* set: single mode	*/
#else
...
#ifdef USE_CURSES
    tcsetattr (0, TCSANOW, &single_char);
    code = get_toke (FALSE);
    tcsetattr (0, TCSANOW, &init_char);
#else
