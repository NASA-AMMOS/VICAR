...
// RadiometryPsyche
//
// subclass of Radiometry model for Psyche cameras.
////////////////////////////////////////////////////////////////////////
#ifndef RADIOMETRYPSYCHE_H
#define RADIOMETRYPSYCHE_H
...
#define PSYCHE_MAX_FLAT_CACHE	500
#define PSYCHE_NUM_SLOTS	5

class RadiometryPsyche: public RadiometryModel {
  ...
  protected:
    ...
    float _flat_parms[5];
    char *_ff_file[PSYCHE_NUM_SLOTS];
    char *_ff_desc[PSYCHE_NUM_SLOTS];
    char *_mode;
    int _nominal_bits;
    int _sample_bits;
    // For ZCAM
    ...
    // Pre-cached flats
    int _slot[PSYCHE_NUM_SLOTS]; // For zcam, 0=rad, 1=fzoom_canon, 2=fzooom_real
		// (zcam) 3=static bias 4=dynamic bias
		// For others, only 0 is used

    // static variables to hold cache of flat field buffer & name
    static RadiometryCalImage *_flat_cache[PSYCHE_MAX_FLAT_CACHE];
    static char *_flat_cache_names[PSYCHE_MAX_FLAT_CACHE];
    ...
    // Note that since Psyche is capable of doing certain 
  ...
  public:
    ...
    static RadiometryPsyche *create(PigFileModel *file);

    RadiometryPsyche(const char *mission, const char *host_id, 
    ...
    virtual ~RadiometryPsyche();

    // These are overridden from the base class to return Psyche specific
    // values
    ...
    //
    // As with MER/MSL/M20, Psyche responsivities are the *inverse* of the way it
    ...
    // other physical units.  For Psyche we convert from
    ...
    virtual const char *const getModelName() { return "RadiometryPsyche"; }
};
