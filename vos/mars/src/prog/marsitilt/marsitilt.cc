...
void main44()
{
    ...
    zvmessage("MARSTILT version 2020-07-01", "");
    ...
    // Delta-tilt is InSight-specific!

    if (delta_tilt && strcasecmp(instType, "SEIS") != 0) {
	zvmessage("Delta-tilt mode can only be used with SEIS instrument","");
	zabend();
    }

    params.iparams_b = NULL;
    if (delta_tilt) {
	params.iparams_b = PigInstrumentParams::create("WTS");
    }
	...
	params.iparams_b->readInstrumentParams();

        std::vector<SampleCircle> points_of_feet;	// SEIS
	params.iparams->plot_inst_feet(points_of_feet);

	std::vector<SampleCircle> points_of_feet_b;	// WTS
	params.iparams_b->plot_inst_feet(points_of_feet_b);

        compute_delta_tilt(params, input_xyz, points_of_feet, 
                          points_of_feet_b, output_min_max_tilt);
    } else {

        std::vector<SampleCircle> points_of_instrument_feet;
	params.iparams->plot_inst_feet(points_of_instrument_feet);
        ...
        int band[3], char *type)
{
    ...
    if (nids == 1) {
        ...
        snprintf(msg, MSG_LEN, "MARSITILT requires either 1 3-band file or 3 single band "
