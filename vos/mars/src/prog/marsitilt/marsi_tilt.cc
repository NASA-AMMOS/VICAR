    ...
    FootPointStruct *fp)
{
	...
	// Find the point closest to the center of the circle
        ...
        const auto index = KDTree_closest_index(kd_tree, x, y, point_epsilon);
	if (index < 0) {
	    okay = false;
	    break;
	}
                  ...
                  PigCoordSystem *cs, PigCoordSystem *p_cs, int do_xpx)
{
    ...
    const PointCloud2D<double> cloud = PigI_xyz_to_point_cloud(input_xyz);
    KDTree kd_tree(
        2,
        cloud,
    ...
    // Force the CS caches to get updated before parallel
    PigPoint pp = cs->convertPointToFixed(PigPoint());
    pp = p_cs->convertPointToFixed(PigPoint());
...
#pragma omp parallel for schedule(dynamic) if (omp_on)
	...
	FootPointStruct *fp = new FootPointStruct(input_xyz,params.foot_window);
		...
		continue;		// no value here
            ...
            // are 0.0.
            double offset_x = params.iparams->getOffsetX(raw_x, raw_y);
            double offset_y = params.iparams->getOffsetY(raw_x, raw_y);
	    ...
	    for (double clock = params.clock_range[0];
			...
			clock += params.clock_step) {
                ...
                const auto new_circles = params.iparams->rotateInstrument(
                    raw_circles, raw_x, raw_y, clock);
                
                //apply offsets if inst is WTS
                const auto circles = params.iparams->applyOffsets(new_circles,
                    offset_x, offset_y);
...
#pragma omp parallel for schedule(dynamic) if (omp_on)
	...
	FootPointStruct *fp = new FootPointStruct(input_xyz,params.foot_window);
		...
		continue;		// no value here
            ...
            double inst_b_offset_x = params.iparams_b->getOffsetX(raw_x, raw_y);
            double inst_b_offset_y = params.iparams_b->getOffsetY(raw_x, raw_y);
	    ...
	    for (double clock = params.clock_range[0];
			...
			clock += params.clock_step) {
                ...
                //apply offsets if inst is WTS
                const auto circles_a = params.iparams->applyOffsets(
		...
		for (double clock2 = params.delta_wts_range[0];
			    ...
			    clock2 += params.delta_wts_step) {
            	    ...
            	    const auto new_instrument_b =
			params.iparams_b->rotateInstrument(
		                raw_instrument_b,
                                raw_x, raw_y,
                                clock2);
       
                    //apply offsets if inst is WTS
                    const auto circles_b = params.iparams_b->applyOffsets(
