...
c
c        sp = a0 + a1*(l-l0) + a2*(s-s0) + a3*(l-l0)*(s-s0)
c        lp = b0 + b1*(l-l0) + b2*(s-s0) + b3*(l-l0)*(s-s0)
c
c The result from the fit is returned in common/cx/
c
      subroutine match_bilinear(nl,ns,I2,l0,s0,lmax,eps,nlw,nsw,
...
c     ...output values
      ...
      real*8 x(8)               !x=[a0,b0,a1,b1,a2,b2]transpose
      real*8 a0,b0,a1,b1,a2,b2,a3,b3
      equivalence (x(1),a0),(x(2),b0),(x(3),a1),(x(4),b1)
      equivalence (x(5),a2),(x(6),b2),(x(7),a3),(x(8),b3)

      real*8 Atr(8),magAtr
      real*8 AtA(8,8),LL(8,8),Atb(8)
...
c     ...Gauss-Newton loop
      ...
      do l=l0-hnlw,l0+hnlw
         ...
         do s=s0-hnsw,s0+hnsw
            i = i + 1
            ds = s - s0
            sp = a0 + a1*dl + a2*ds + a3*dl*ds
            lp = b0 + b1*dl + b2*ds + b3*dl*ds
     ...
     &		lp.lt.2 .or. lp.ge.nl) goto 990
		...
		!dI2/dsp = 0.5*(I2(lp,sp+1)-I2(lp,sp-1))
            ...
            A(i,7) = -dI2dsp*dl*ds      !dr/da3
            A(i,8) = -dI2dlp*dl*ds      !dr/db3
         enddo
      enddo
...
c     ...radiometric fit
      ...
      rho = -999.
      if (varI2.le.0.) return		!no image info in target window
...
c      write(msg,49) loop,(x(i),i=1,n)
c   49 format(i2,' x=',10f11.5)
c      call xvmessage(msg,' ')
   50 continue
