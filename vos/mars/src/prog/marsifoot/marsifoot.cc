...
void main44()
{
    ...
    // User Parameters
    PigInstrumentParams *iparams, *iparams2;
    ...
    // Special case: BOTH means SEIS and WTS

    if (strcasecmp(instType, "BOTH") == 0) {
	iparams = PigInstrumentParams::create("SEIS");
	iparams2 = PigInstrumentParams::create("WTS");
	do_both = TRUE;
    } else{
        iparams = PigInstrumentParams::create(instType);
	iparams2 = NULL;
	do_both = FALSE;
    }
...
printf("count=%d def=%d\n", count, def);	//!!!!
	...
	if (loop == 1) {	// for "both" case
	    iparams = iparams2;		// yes this wipes out iparams
	...
	const std::vector<SampleCircle> feet =
		plot_points_of_instrument_feet(iparams, do_body);

        // Convert radial and cross-radial offsets to x and y offsets
        // if inst is WTS. If it is not WTS, then offset_x and offset_y
        // are 0.0.
        double offset_x = iparams->getOffsetX(xy[0], xy[1]);
        double offset_y = iparams->getOffsetY(xy[0], xy[1]);
	...
	for (double clock = clock_range[0];
	    ...
	    const auto new_feet = iparams->rotateInstrument(
	        feet, xy[0], xy[1], clock);

            //apply offsets if inst is WTS
            const auto circles = iparams->applyOffsets(new_feet, offset_x, 
                offset_y);
        ...
        int band[3], char *type)
{
    ...
    if (nids == 1) {
        ...
        snprintf(msg, msgLen, "MARSIFOOT requires either 1 3-band file or 3 single band files as input for %s", type);
        zvmessage(msg, "");
        zabend();
    }
}
