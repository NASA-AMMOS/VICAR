...
{
    InsightInstrument inst;	// Instrument: SEIS or WTS
    InstrumentParams iparams;	// params for instrument footprints
    int leaf_max_size;
    double point_epsilon;
    double sinkage;		// settling value to use

    double seis_body_thresh;
    double seis_feet_thresh;
    double wts_body_thresh;
    double wts_feet_thresh;
    double hp3_body_thresh;
    double hp3_feet_thresh;

    double clock_range[2];
    double clock_step;
    double hp3_body_step;
