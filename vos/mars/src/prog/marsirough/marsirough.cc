...
void main44()
{
    ...
    char instType[20];
    zvp("INST", instType, &count);

    params.iparams = PigInstrumentParams::create(instType);
    ...
    // Thresholds.  We have a general thresh and then one for each
    // instrument.  The inst ones replace the general one if given -
    // that way we don't have to keep specifying them when running

    zvparmd("BODY_THR", &params.body_thresh,&count,&def,1,0);
    zvparmd("FEET_THR", &params.feet_thresh,&count,&def,1,0);

    if (strcasecmp(instType, "HP3") == 0) {
	double temp;
        zvparmd("HP3_BODY_THR", &temp,&count,&def,1,0);
	if (count != 0)
	    params.body_thresh = temp;
        zvparmd("HP3_FEET_THR", &temp,&count,&def,1,0);
	if (count != 0)
	    params.feet_thresh = temp;
    }
    if (strcasecmp(instType, "SEIS") == 0) {
	double temp;
        zvparmd("SEIS_BODY_THR", &temp,&count,&def,1,0);
	if (count != 0)
	    params.body_thresh = temp;
        zvparmd("SEIS_FEET_THR", &temp,&count,&def,1,0);
	if (count != 0)
	    params.feet_thresh = temp;
    }
    if (strcasecmp(instType, "WTS") == 0) {
	double temp;
        zvparmd("WTS_BODY_THR", &temp,&count,&def,1,0);
	if (count != 0)
	    params.body_thresh = temp;
        zvparmd("WTS_FEET_THR", &temp,&count,&def,1,0);
	if (count != 0)
	    params.feet_thresh = temp;
    }
    ...
    params.body_step = 1.0;

    if (strcasecmp(instType, "HP3") == 0) {
        zvparmd("HP3_BODY_STEP", &params.body_step, &count, &def, 1, 0);
        if (count == 0) {
	    params.body_step = (params.clock_range[1]-params.clock_range[0])/2;
	    if (params.body_step < EPSILON)
	        params.body_step = .01;		// just make sure no inf loop
	    snprintf(msg, msgLen, "Setting HP3 body clock step to %f",
					params.body_step);
	    zvmessage(msg, "");
	}
    }
    ...
    PigCoordSystem *center_cs = cs;
    char center_coord[256];
    zvp("CENTER_COORD", center_coord, &count);
    if (count != 0) {
        center_cs = m->getCoordSystem(file_models[0], center_coord);
	if (center_cs == NULL) {
	    zvnabend(256, "Invalid CENTER_COORD: %s", center_coord);
	}
	PigPoint ctr(params.x_center, params.y_center, 0);
	PigPoint new_ctr = cs->convertPoint(ctr, center_cs);
	params.x_center = new_ctr.getX();
	params.y_center = new_ctr.getY();
    }

    int no_feet = zvptst("NO_FEET");
    ...
    snprintf(msg, msgLen, "******** COMPUTING %s BODY ********", instType);
    zvmessage(msg, "");

    // If we are rotationally symmetric for the body, we don't need
    // to bother with clocking

    double clock[2] = {0.0, 0.0};

    int sym = params.iparams->isBodyRotSymmetric();
    if (!sym) {
	clock[0] = params.clock_range[0];
	clock[1] = params.clock_range[1];
    }

    // Compute footplane

    params.iparams->readInstrumentShape(shape, INST_BODY);
    computeHillRoughness(params, xyz_img, uix, zix, shape, rough[INST_BODY],
			clock, params.body_step);

    if (!no_feet) {
	snprintf(msg, msgLen, "******** COMPUTING %s FEET ********", instType);
	zvmessage(msg, "");

	// Compute footpatch.  Note that we use the same normal for
	// both; it doesn't matter that much for the footpatch case.

	params.iparams->readInstrumentShape(shape, INST_FEET);
    ...
    double thresh[2];
    thresh[0] = params.body_thresh;
    thresh[1] = params.feet_thresh;
    int rb = rough_bands;
    if (no_feet)
	rb = 1;			// ignore the foot band
    computeState(params, rough, rb, thresh, state);
        ...
        int band[3], char *type, int nbands)
{
	...
	if (nbands == 3)
           snprintf(msg, msgLen, "MARSIROUGH requires either 1 3-band file or 3 single band files as input for %s", type);
	else if (nbands == 1)
           snprintf(msg, msgLen, "MARSIROUGH requires one single band file as input for %s", type);
	else
