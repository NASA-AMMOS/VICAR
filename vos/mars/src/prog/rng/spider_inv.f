...
c An object is a region whose pixels are above a threshold.
c img,nl,ns,nmax,i0,j0,thresh are inputs.  All other arguments are outputs.
c
      subroutine spider_inv(img,nl,ns,vx,vy,nmax,i0,j0,thresh,n)
      implicit none
  ...
  101 format(4i8)
    ...
    1 if (img(i+1,j-1).gt.thresh) goto 10
    2 if (img(i,j-1)  .gt.thresh) goto 20
    3 if (img(i-1,j-1).gt.thresh) goto 30
    4 if (img(i-1,j)  .gt.thresh) goto 40
    5 if (img(i-1,j+1).gt.thresh) goto 50
    6 if (img(i,j+1)  .gt.thresh) goto 60
    7 if (img(i+1,j+1).gt.thresh) goto 70
    8 if (img(i+1,j)  .gt.thresh) goto 80
      if (img(i+1,j-1).gt.thresh) goto 10
      if (img(i,j-1)  .gt.thresh) goto 20
      if (img(i-1,j-1).gt.thresh) goto 30
      if (img(i-1,j)  .gt.thresh) goto 40
      if (img(i-1,j+1).gt.thresh) goto 50
      if (img(i,j+1)  .gt.thresh) goto 60
      if (img(i+1,j+1).gt.thresh) goto 70
