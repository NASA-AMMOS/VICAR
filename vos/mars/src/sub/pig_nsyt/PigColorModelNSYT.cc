...
PigColorModelNSYT::PigColorModelNSYT(const char *mission, 
    ...
    : PigColorModel(mission, instrument)
{
    ...
    if (constructPrefix()) {
        ...
        char *config_file = "param_files/NSYT_flat_fields.parms";
        ...
        if (pcc_xyz_to_prgb->isValid())
            _pcc_array[_count++] = pcc_xyz_to_prgb;
        if (pcc_rad_to_irgb->isValid())
            _pcc_array[_count++] = pcc_rad_to_irgb;
    }
}

PigColorModelNSYT::PigColorModelNSYT() : PigColorModel()
{
    _host_id = _prefix = NULL; 
}
...
PigColorModelNSYT::~PigColorModelNSYT()
{
...
PigColorModelNSYT *PigColorModelNSYT::create(PigFileModel *file) 
{
    PigMission *m = PigMission::getMissionObject(file);
    return new PigColorModelNSYT(file->getMissionName(), 
...
int PigColorModelNSYT::constructPrefix()
{
...
double PigColorModelNSYT::getStaticColorScalingFactor(char *config_file, 
                                                      char *prefix)
{
    FILE *inClientFile;
    char pre[256];
