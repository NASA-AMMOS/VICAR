...
// RadiometryPsyche
//
// Subclass for Psyche Cameras Radiometry Model.
...
RadiometryCalImage *RadiometryPsyche::_flat_cache[PSYCHE_MAX_FLAT_CACHE];
char *RadiometryPsyche::_flat_cache_names[PSYCHE_MAX_FLAT_CACHE];
int RadiometryPsyche::_flat_cache_init = 0;
...
RadiometryPsyche::RadiometryPsyche(const char *mission,
			     const char *host_id,
...

{
    ...
    for (int i=0; i < PSYCHE_NUM_SLOTS; i++) {
        _ff_file[i] = NULL;
        _ff_desc[i] = NULL;
    }
    ...
    if (!_flat_cache_init) {
	for (int i=0; i < PSYCHE_MAX_FLAT_CACHE; i++) {
    ...
    read_flat_field_parms("param_files/Psyche_rad_cal.parms");
...
RadiometryPsyche::RadiometryPsyche() : RadiometryModel()
{
    strcpy(_host_id,"");
    strcpy(_instrument, "");
    strcpy(_filter, "");
...
RadiometryPsyche::~RadiometryPsyche()
{
    for (int i=0; i<PSYCHE_NUM_SLOTS; i++) {
        if (_ff_file[i] != NULL)
	    delete _ff_file[i];
...
RadiometryPsyche *RadiometryPsyche::create(PigFileModel *file)
{
    ...
    return new RadiometryPsyche(file->getMissionName(),
			     m->getHostID(),
...
void RadiometryPsyche::loadFlatFields(const char *mode)
{
    if (_use_zcam_flat) {
...
int RadiometryPsyche::loadFlatField(const char *mode, const char *type, int *pos,
				int index)
{
	...
	int slot = getCacheSlot(fn, _flat_cache_names, PSYCHE_MAX_FLAT_CACHE);
...
char *RadiometryPsyche::readVarFlat(const char *fn, int &zoom_value)
{
    char fn_path[1024];
...
//
////////////////////////////////////////////////////////////////////////
void RadiometryPsyche::applyCorrectionInternal(void *image, int max_nl, int max_ns,
...
float RadiometryPsyche::getFlatFieldDN(RadiometryCalImage *flat,
...
//
// As with MER/MSL/M20, Psyche responsivities are the *inverse* of the way it was
...
double RadiometryPsyche::getResponsivityFactor(int band)
{
...
double RadiometryPsyche::getResponsivity(int band)
{
...
void RadiometryPsyche::read_flat_field_parms(char *filename)
{
    FILE *inClientFile;
    char line[255];
    char fmt_sn[200];
