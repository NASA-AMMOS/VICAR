...
// RadiometryMSL
//
// Subclass for MSL Cameras Radiometry Model.
...
RadiometryCalImage *RadiometryMSL::_flat_cache[MSL_MAX_FLAT_CACHE];
char *RadiometryMSL::_flat_cache_names[MSL_MAX_FLAT_CACHE];
int RadiometryMSL::_flat_cache_init = 0;

////////////////////////////////////////////////////////////////////////
// Constructor
////////////////////////////////////////////////////////////////////////

RadiometryMSL::RadiometryMSL(const char *mission,
			     const char *host_id,
...

{
    ...
    if (!_flat_cache_init) {
	for (int i=0; i < MSL_MAX_FLAT_CACHE; i++) {
    ...
    read_flat_field_parms("param_files/MSL_flat_fields.parms");
    loadFlatField(_mode);		// populate the id for the label
}

RadiometryMSL::RadiometryMSL() : RadiometryModel()
{
    strcpy(_host_id,"");
    strcpy(_instrument, "");
    strcpy(_filter, "");
...
RadiometryMSL *RadiometryMSL::create(PigFileModel *file)
{
    ...
    return new RadiometryMSL(file->getMissionName(),
			     m->getHostID(),
...
int RadiometryMSL::loadFlatField(const char *mode)
{
	...
	int slot = getCacheSlot(fn, _flat_cache_names, MSL_MAX_FLAT_CACHE);
	...
	if (_flat_cache[slot]->_product_desc != NULL) {
	    ...
	    _ff_desc = NULL;		// probably not needed
	}
...
////////////////////////////////////////////////////////////////////////
void RadiometryMSL::applyCorrectionInternal(void *image, int max_nl, int max_ns,
					int is_float, int band, int hasBeenRad)
{
	...
	for (j=0; j < (_es - _ss + 1); j++) {
	    ...
	    double flat_dn;
		    ...
		    // output.  This lets the corners of e.g. RMI go transparent
...
float RadiometryMSL::getFlatFieldDN(RadiometryCalImage *flat,
...
double RadiometryMSL::getResponsivityFactor(int band)
{
    if (_color) {
...
double RadiometryMSL::getResponsivity(int band)
{   
...
void RadiometryMSL::read_flat_field_parms(char *filename)
{
    FILE *inClientFile;
    char line[255];
