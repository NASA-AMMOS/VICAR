...
// RadiometryColdarm
//
// Subclass for Coldarm Cameras Radiometry Model.
...
RadiometryCalImage *RadiometryColdarm::_flat_cache[COLDARM_MAX_FLAT_CACHE];
char *RadiometryColdarm::_flat_cache_names[COLDARM_MAX_FLAT_CACHE];
int RadiometryColdarm::_flat_cache_init = 0;
...
RadiometryColdarm::RadiometryColdarm(const char *mission,
			     const char *host_id,
...

{
    ...
    for (int i=0; i < COLDARM_NUM_SLOTS; i++) {
        _ff_file[i] = NULL;
        _ff_desc[i] = NULL;
    }
    ...
    if (!_flat_cache_init) {
	for (int i=0; i < COLDARM_MAX_FLAT_CACHE; i++) {
    ...
    read_flat_field_parms("param_files/Coldarm_rad_cal.parms");
...
RadiometryColdarm::RadiometryColdarm() : RadiometryModel()
{
    strcpy(_host_id,"");
    strcpy(_instrument, "");
    strcpy(_filter, "");
...
RadiometryColdarm::~RadiometryColdarm()
{
    for (int i=0; i<COLDARM_NUM_SLOTS; i++) {
        if (_ff_file[i] != NULL)
	    delete _ff_file[i];
...
RadiometryColdarm *RadiometryColdarm::create(PigFileModel *file)
{
    ...
    return new RadiometryColdarm(file->getMissionName(),
			     m->getHostID(),
...
void RadiometryColdarm::loadFlatFields(const char *mode)
{
    if (_use_zcam_flat) {
...
int RadiometryColdarm::loadFlatField(const char *mode, const char *type, int *pos,
				int index)
{
	...
	int slot = getCacheSlot(fn, _flat_cache_names, COLDARM_MAX_FLAT_CACHE);
...
char *RadiometryColdarm::readVarFlat(const char *fn, int &zoom_value)
{
    char fn_path[1024];
...
//
////////////////////////////////////////////////////////////////////////
void RadiometryColdarm::applyCorrectionInternal(void *image, int max_nl, int max_ns,
...
float RadiometryColdarm::getFlatFieldDN(RadiometryCalImage *flat,
...
//
// As with MER/MSL/M20, Coldarm responsivities are the *inverse* of the way it was
...
double RadiometryColdarm::getResponsivityFactor(int band)
{
...
double RadiometryColdarm::getResponsivity(int band)
{
...
void RadiometryColdarm::read_flat_field_parms(char *filename)
{
    FILE *inClientFile;
    char line[255];
    char fmt_sn[200];
