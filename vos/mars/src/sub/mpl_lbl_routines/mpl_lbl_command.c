 ...
 *  MplLblCommand.  This routine requires exactly 4 parameters.
...
#define  LBL_SIZE(x)	sizeof(((MplLblCommand_typ *)0)->x)

static LblApiElement_typ	LabelTbl[] = {
	{"LAMP_STATE_ID",			"STRING",	LBL_OPTIONAL,
		LBL_NO_CONT,	1,	1,	LBL_NULL,
		LBL_OFFSET(MplLblCommand_typ, LampStateId.Value),
		LBL_OFFSET(MplLblCommand_typ, LampStateId.Valid),
		LBL_NO_RETURN,	LBL_SIZE(LampStateId.Value)},

	{"REQUEST_DESCRIPTION",			"STRING",	LBL_OPTIONAL,
		LBL_NO_CONT,	1,	1,	LBL_NULL,
		LBL_OFFSET(MplLblCommand_typ, RequestDescription.Value),
		LBL_OFFSET(MplLblCommand_typ, RequestDescription.Valid),
		LBL_NO_RETURN,	LBL_SIZE(RequestDescription.Value)},

	{"REQUEST_ID",				"STRING",	LBL_REQUIRED,
		LBL_NO_CONT,	1,	1,	LBL_NULL,
		LBL_OFFSET(MplLblCommand_typ, RequestId.Value),
		LBL_OFFSET(MplLblCommand_typ, RequestId.Valid),
		LBL_NO_RETURN,	LBL_SIZE(RequestId.Value)},

	{"REQUEST_NAME",			"STRING",	LBL_REQUIRED,
		LBL_NO_CONT,	1,	1,	LBL_NULL,
		LBL_OFFSET(MplLblCommand_typ, RequestName.Value),
		LBL_OFFSET(MplLblCommand_typ, RequestName.Valid),
		LBL_NO_RETURN,	LBL_SIZE(RequestName.Value)},
...
int     MplLblCommand(
  int   Unit,
  int   Obtain,
  MplLblCommand_typ      *LabelItems,
  int	Instance)
{ int   RtnStatus;
  LblApiCntrl_typ	Cntrl;

  Label.Buffer = (void *)LabelItems;
  Label.BufferSize = sizeof(MplLblCommand_typ);
