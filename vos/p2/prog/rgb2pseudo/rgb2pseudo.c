...
int merge(struct entry_def *table,int num_colors);
int make_composite(struct entry_def *table,unsigned char *red_lut,
  unsigned char *green_lut,unsigned char *blue_lut,
  unsigned char *composite,int nl,int ns,int bits_used);
int rgb_to_image_and_luts(unsigned char *red,unsigned char *green,
  unsigned char *blue,unsigned char *composite,unsigned char *red_lut,
  unsigned char *green_lut,unsigned char *blue_lut,int nl,int ns,
  int bits_used);
int allocate_space(struct entry_def **table_ptr,
  struct pix_list_def **pix_space_ptr,int bits_used,int nl,int ns,
  int *length_ptr);
int build_table(unsigned char *red,unsigned char *green,
  unsigned char *blue,struct entry_def *table,
  struct pix_list_def *pix_space, int length, int nl, int ns, int bits_used);
int compare(struct entry_def *entry1,struct entry_def *entry2);
void mybcopy(char *src, char *dst, int len);
void mybzero(char* buf, int len);
void output_message_char(char* msg);
void output_message_int1(char* msg, int p1);
void output_message_int8(char* msg, int p1, int p2, int p3, int p4, 
  int p5, int p6, int p7, int p8);
int ShellSort(char *start,long nelem, int size, int	(*compar)());
...
int rgb_to_image_and_luts(red,green,blue,composite,red_lut,green_lut,blue_lut,nl,ns,bits_used)
...
int nl,ns,bits_used;
{
    ...
    (void)output_message_char("building histogram");
...
int allocate_space(table_ptr,pix_space_ptr,bits_used,nl,ns,length_ptr)
...
int build_table(red,green,blue,table,pix_space,length,nl,ns,bits_used)
unsigned char *red,*green,*blue;
struct pix_list_def *pix_space;
...
int length,nl,ns,bits_used;
{
    ...
    (void)output_message_int1("found %d colors",next_free_entry);
...
int make_composite(table,red_lut,green_lut,blue_lut,composite,nl,ns,bits_used)
struct entry_def *table;
...
int nl,ns,bits_used;
{
    ...
    (void)output_message_char("creating composite image");
    for (dn=0;dn < 256;dn++) {
...
void output_message_char(msg)
  char *msg;
{
    output_message_int8(msg,0,0,0,0,0,0,0,0);
}

void output_message_int1(msg, p1)
  char *msg;
  int	p1;
{
  output_message_int8(msg,p1,0,0,0,0,0,0,0);
}

void output_message_int8(msg, p1, p2, p3, p4, p5, p6, p7, p8)
  char *msg;
