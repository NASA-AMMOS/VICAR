...
{ 
  ...
  int true_ns=0,true_nl=0, small_sl, small_ss, small_nl, small_ns;
  ...
  long unsigned int total1=0;
  long unsigned int cnt1=0;

  struct PARBLK parb; /* output angle to vicar pdf parmameter */
  ...
  **********/
  ...
  if (ptcnt > 2) fitpts(x,y,nx,ny,ptcnt,&fitpts_nndata,&fitpts_mean,
    ...
    fprintf(stderr,"(check parameters and/or input image %s)\n\n", inp);
    zabend();
    /*****
	  exit(9);
    ******/
		 ...
		 unsigned int nl,unsigned int ns,unsigned int skip)
{
  ...
  unsigned int beg=0, end=0, mid=0, *edgearray;
...
{
  double mean=0,/*sigma,*/ /**x[1500],y[1500],absres,**/res[1500];
  long int sx,sy,sxy,sxx,sumres,sres,absres;
  /**	double sumres,sres;*/
  double maxdiff,newy,a,b=0/**,ny[1500],nx[1500]**/;
  /*	long int maxdiff;
