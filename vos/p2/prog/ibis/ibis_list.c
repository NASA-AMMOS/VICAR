...
#define IBISPRINT( str, bufsiz, ptr, fmtchar, fmtStr )	\
	switch (tolower(fmtchar)) { \
	case 'b': snprintf( str, bufsiz, fmtStr,    *(char *)  (ptr));  break; \
	case 'h': snprintf( str, bufsiz, fmtStr,    *(short *) (ptr));  break; \
	case 'f': snprintf( str, bufsiz, fmtStr,    *(long *)  (ptr));  break; \
	case 'r': snprintf( str, bufsiz, fmtStr,    *(float *)  (ptr));  break; \
	case 'd': snprintf( str, bufsiz, fmtStr,    *(double *)(ptr));  break; \
	case 'c': snprintf( str, bufsiz, fmtStr,    ((float*)ptr)[0],((float*)ptr)[1] );break; \
	case 'a': snprintf( str, bufsiz, fmtStr,    ptr );break;	\
	default:  snprintf( str, bufsiz, fmtStr,   "BAD FORMAT!" );break; \
	}
...
#define IBISFORMAT( str, fmtchar, colsize ) \
  ...
  case 'b':				    \
  case 'h':								\
  case 'f': snprintf( str, 20, "%%%dd",  (colsize));  break;		\
  case 'r': snprintf( str, 20, "%%%d.2f",  (colsize));  break;		\
  case 'd': snprintf( str, 20, "%%%d.2lf", (colsize));  break;		\
  case 'c': snprintf( str, 20, " (%%-%d.2f,%%%d.2f)",			\
		     ((colsize)-4)/2, colsize - (4 + ((colsize)-4)/2) ); \
    break;								\
  case 'a': snprintf( str, 20, "%%%ds", (colsize) );break;		\
  default:  snprintf( str, 20, "%%%ds", (colsize));break;		\
  }
...
void list_file(void)
{
	...
	snprintf(headerfmt, 20, "C:%%-%pd", csize-2);
	snprintf(formatfmt, 20, "%%-%ps", csize);
	...
	for (col=0; col<nc; col+=colperrow)
	{
		...
		for (colnum = 0; colnum<colperrow; colnum++)
		{
			...
			snprintf(groupStr+coffset[colnum], MAXCOLSIZE+1-coffset[colnum],
				 "%-*s",csize[col+colnum], grpelemt);

			strcpy(unitelemt," -- ");
			IBISGroupFind( in, ITYPE_UNIT, colm, unitelemt, 1,1,sizeof(unitelemt));
			snprintf(unitStr+coffset[colnum], MAXCOLSIZE+1-coffset[colnum],
			...
			snprintf(fmtelemt, MAXCOLSIZE+1, "C:%-d", colm);
			snprintf(headerStr+coffset[colnum], ROWSIZE+2-coffset[colnum],
				 "%*s",csize[col+colnum], fmtelemt);

			snprintf(fmtelemt, MAXCOLSIZE+1, "%*s",csize[col+colnum], colformat);
		...
		for (rows_left=nr; rows_left>0; rows_left-=rinc)
		{
			...
			{
				outmessage( " ", " ");
				snprintf( outStr, ROWSIZE+2, "Rows: %-d:%-d", srow1, srow1+rinc-1 );
				outmessage( outStr, " ");
				outmessage( lineStr, " " );
			...
			for (row=0; row<rinc; row++)
			{
				...
				{
					colm = ccnt ? cols[col+colnum] : col+scol+colnum ;
					IBISPRINT ( valueelemt, MAXCOLSIZE+1, 
...
int *csize;
{
	...
	for (ptr=userfmt; *ptr; ptr=endpt,csize++,index++)
	{
		...
		fmt[len]='\0';
                if (!charpt || charpt>endpt)
		  snprintf(test, 100, fmt,0.0);
                else
		  snprintf(test, 100, fmt,"woof");
		*csize = strlen(test);
...
char *version;
{
	...
	outmessage( " ", " ");
	snprintf( outStr, 200, "Number of Rows:%-d  Number of Columns: %-8d", nr, nc );
	outmessage( outStr, " ");
	snprintf( outStr, 200, "File Version:%s  Organization:%s  SubType:%s",
		 version, org, type );
	outmessage( outStr, " ");
}
...
char *type;
{
	...
	for (group=0;group<ngroups;group++)
	{
		...
		snprintf( outStr, 200, fmtstr, groupname );
		...
		for (i=0;i<ngcols;i++)
		{
		  snprintf(valueelemt, 200, " %d", cols[i]);
