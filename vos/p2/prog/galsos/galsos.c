...
void main44()
{
  ...
  zvmessage("GALSOS version 2019-07-23","");
  debug = zvptst("DEBUG");
     ...
     int ifr,dc_fr,brate;		/* frame rates for image, dc, blem */
{
  ...
  zvmessage("              ===   ===   ==   ===","");
  zvnprintf(80, " FILTER       %3d   %3d    X   %3d",
	    ifilt,  /* Image Filter */
	    cfilt,  /* CAL Filter   */
	    bfilt); /* BLEM Filter  */
  zvnprintf(80, " GAIN         %3d     X   %2d   %3d",
	    igain, dc_gain, bgain);
  zvnprintf(80, " FRAME-RATE   %3d     X  %3d     X", ifr, dc_fr);
  zvmessage(" ","");
...
static void copy_binary_headers()
{
  ...
  if (mission_phase == PHASE1) {
    ...
    snprintf(msg, MSG_LEN, "%8.4f", a1);
    memcpy(tlm_hdr.scale[0],msg,8);
    snprintf(msg, MSG_LEN, "%8.4f", a2);
...
int decalibrate_image()
{
  ...
  short *next_line = NULL;	/* output pixels of next line     */ 
  ...
  t=gll.exposure;
  zvnprintf(MSG_LEN, " Exposure = %.4e msec", t);
  ...
  for (line=1; line<=nl; line++) {
	...
	if (errmsg==0) zvmessage(" ***Segment field is invalid", "");
	errmsg = 1;
	ss1 = 1;
	es1 = ns;
	ss2 = 0;
     ...
     able_86_typ *gll;
{
  ...
  char format[5];
  char filespec[FILESPEC_LEN];
  able_86_typ blem_able;
  char *tempfilename;

  memset(filespec,'\0',FILESPEC_LEN);
     ...
     int *mission_phase;
{
  ...
  if (idef != 1) {
    ...
    zvnprintf(MSG_LEN, "Solar range from user parameter=%11.1f\n", gll->sol_range);
    return;
  }

  if (*mission_phase==PHASE2 && gll->sol_range > 0.0) {
    zvnprintf(MSG_LEN, "Solar range from image label=%11.1f\n", gll->sol_range);
    return;
  }
  ...
  if (i<0 || i==4 || i>5) {
    ...
    zvmessage("***Replacing target with planet-of-encounter","");
  }
  ...
  if (status==1 && buf[24]>0.) {
    zvnprintf(MSG_LEN, "Solar range extracted from SPICE=%11.1f\n", gll->sol_range);
    zvnprintf(MSG_LEN, "Target=%s\n", gll->trgt_nm);
    return;
  }
  
  gll->sol_range = range[i]*KM_PER_AU;
  zvnprintf(MSG_LEN, "Solar range copied from table=%11.1f\n", gll->sol_range);
  strcpy(gll->trgt_nm,planet_name[i]);
  zvnprintf(MSG_LEN, "Target=%s\n", gll->trgt_nm);
  return;
}
     ...
     able_86_typ *gll;
{
  ...
  zvnprintf(MSG_LEN, " Reflectivity per Foot-Lambert = %.4f", conv1);
  zvnprintf(MSG_LEN, " Radiance per Foot-Lambert = %.4f", conv2);
  ...
  if (idef == 0) {
    ...
    *scale = (conv2/(*a2))*((gain_ratio[igain-1])/(gain_ratio[cgain-1])); 
    ...
    gll->iof = *a1;    
    rtn_val = 2;
    zvnprintf(MSG_LEN, " Scaling values to be used = %.4e ", *scale);
  }
  else {
    zvp("IOF",a1,&icnt);
     ...
     int line;			/* current line number in image	*/
{
  ...
  for (;(current_blem <= last_blem) && (line == current_blem->line);
    ...
    default : 
      zvnprintf(MSG_LEN," Blem class (%d); pixel (%d,%d) not corrected.",
     ...
     float *entropy;		/* output image entropy in bits/DN */
{
  ...
  if (zvptst("PRINT")) {	 /* Print Entropy results */
    zvnprintf(MSG_LEN, "Image Entropy: %7.4f", *entropy);
    for (i=0; i<15; i++) {
      zvnprintf(MSG_LEN, "Line %03d Entropy: %7.4f", (i+1)*50, entropies[i]);
    }
  }
  if (calc_entropy) {
  ...
  zvnprintf(MSG_LEN, " GALSOS replaced %d pixels by interpolation", c_blem_cntr);
