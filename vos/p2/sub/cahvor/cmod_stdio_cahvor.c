/******************************************************************************
*                                                                             *
*                        C M O D _ C A H V O R _ S T D I O                    *
*                                                                             *
*                                       Todd Litwin                           *
*                                       Written:  3 Aug 1993                  *
*                                       Updated: 24 May 2016                  *
...
*******************************************************************************
	...
	This file contains standard I/O functions for the camera model known
...
#define scanstr_(fp, str) \
    if (cmod_cahvor_read2_scanstr(fp, str) == FAILURE) { \
	CMOD_ERROR_S("cmod_cahvor_read2", \
...
cmod_stat_t cmod_cahvor_read2_scanstr(FILE *fp, const char *str);
...
{
    return cmod_cahvor_read2(filename, (cmod_int_t *)NULL, (cmod_int_t *)NULL,
    ...
    cmod_float_t s_int[5][5])	/* output covariance matrix, or NULL */
{
	...
	CMOD_ERROR_S("cmod_cahvor_read2", "Error opening CAHVOR file: ",
		filename);
	return FAILURE;
	}
	...
	*xdim = -1;
	*ydim = -1;
	if (cmod_cahvor_read2_scanstr(fp, "Dimensions =") == FAILURE)
	    rewind(fp);
	else
    ...
    /*scanstr_(fp, "S =");*/
    if (s != NULL) {
	if (cmod_cahvor_read2_scanstr(fp, "S =") == FAILURE) {
	    for (i=0; i<18; i++)
		for (j=0; j<18; j++)
		    s[i][j] = 0;
	    }
	    ...
	    for (i=0; i<18; i++) {
		scan_(fp, 18, (fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\
    ...
    /*scanstr_(fp, "Hs    =");*/
    if (cmod_cahvor_read2_scanstr(fp, "Hs    =") == FAILURE)
    ...
    if (s_int != NULL) {
	if (cmod_cahvor_read2_scanstr(fp, "S internal =") == FAILURE) {
	    for (i=0; i<5; i++)
		for (j=0; j<5; j++)
		    s_int[i][j] = 0;
	    }
    ...
    cmod_float_t s_int[5][5])	/* input covariance matrix, or NULL */
{
	...
	CMOD_ERROR_S("cmod_cahvor_write2", "Error creating CAHVOR file: ",
		filename);
	return FAILURE;
	}
