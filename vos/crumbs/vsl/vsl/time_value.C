...
//printf("%d %d %d %d %d %d %d %d %d\n",fract,sec,min,hour,day,month,year,tz,dst);
//printf("my_time_t = %d\n",my_time_t);
              ...
              int day_of_year, int year)
{
   ...
   day_of_year++;		// starts at 0 for Jan. 1
...
//printf("%d %d %d %d %d %d\n",fract,sec,min,hour,day_of_year,year);
//printf("my_time_t = %d\n",my_time_t);
              ...
              int *day_of_year, int *year, int *tz, int *dst)
{
   ...
   *min = my_time_tm->tm_min;
   *hour = my_time_tm->tm_hour;
   *day_of_year = my_time_tm->tm_yday;
   *year = my_time_tm->tm_year + 1900;
   *dst = my_time_tm->tm_isdst;
...
char *ScEventTime::get_units_label(TimeUnits::units lunits)
{
	static char* labels[] = {"SCET(Year)", "SCET(Day)", "SCET(Hour)", "SCET(Min)", "SCET(Sec)", "SCET(Msec)"};

	return(labels[lunits]);
}
...
char *ScEventTime::get_scet_time(TimeUnits::units tu)
{
	...
	switch(tu) {
	   ...
	   case TimeUnits::DOY:
	      ...
	      break;
	   case TimeUnits::HR:
	      sprintf(scet, "%04d:%03d:%02d", year, doy, hr);
	      break;
   	   case TimeUnits::MIN:
	      sprintf(scet, "%04d:%03d:%02d:%02d", year, doy, hr, min);
	      break;
 	   case TimeUnits::SEC:
	      sprintf(scet, "%04d:%03d:%02d:%02d:%02d", year, doy, hr, min, 
					sec);
	      break;
	   case TimeUnits::MSEC:
	      sprintf(scet, "%04d:%03d:%02d:%02d:%02d.%03d", year, doy, hr, 
					min, sec, msec);
	      break;
	}
