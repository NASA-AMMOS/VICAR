    ...
    for sl, dl in zip(list_sl, demlist_sl):
        for ss, ds in zip(list_ss, demlist_ss):
            r2lib_cmd = (
                ...
                "/BSM_000000_0000_" + out_prod + "_L" + sl + "_S" + ss + "_" +
...
def make_mesh_call(prod_dir, out_prod, demlist_sl, demlist_ss, list_sl,
    ...
    for d_sl, sl in zip(demlist_sl, list_sl):
        ...
        for d_ss, ss in zip(demlist_ss, list_ss):
            ...
            make_mesh_cmd = (
                #"make_mesh.sh " + prod_dir + "BSM_000000_0000_DEM_L" + d_sl +
                os.environ["CRUMBS_HOME"] + "/orbital/make_mesh.sh " + prod_dir + "BSM_000000_0000_DEM_L" + d_sl +
                "_S" + d_ss + "_" + pad_d_size + "_" + pad_d_size + ".VIC " +
                prod_dir + "BSM_000000_0000_" + out_prod + "_L" + sl +
...
def merge_mesh_call(work_dir, skin, sol, site, drive, location_dir=None):
    ...
    if skin == "ESP":
        C = "C"
    meshlist = glob.glob(
        work_dir + "oss/mesh/BSM_000000_0000_" + skin + "*.iv")
    print(work_dir + "oss/mesh/BSM_000000_0000_" + skin + "*.iv")
    meshlist.sort()
    listname = (
        "O_N" + sol + "_ORR_F_" + site + "_" + drive +
        "_" + C + "_BSM25M1.LIS")
...
def main(solsitedrive, dem_inp='DEM', skin_size=1024, req_type='ods', ovr_list=[],
         bigger=None, outputdir=None, skin_list=['ESP', 'BWP'], o_loc=None):
    ...
    # THIS IS ONLY COMMENTED OUT FOR TESTING
