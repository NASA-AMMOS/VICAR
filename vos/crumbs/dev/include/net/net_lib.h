...
// friend classes declared only to allow access to task_id field
	...
	friend class Network_Host;
	friend class Generic_Pool_Of_Tasks;
...
class Message_Passing_Task : virtual public Generic_Distributed_Task {
	...
	int	send_this_buffer(int bufid, int tag = 0) {
			pvm_setsbuf(bufid);
			return(pvm_send(task_id ,tag));
		}
...
//********************** Peer Class for peer-to-peer messaging *******

class Peer_Task : public Message_Passing_Task {

    public:
	Peer_Task(int tid) { task_id = tid; }
...
class Generic_Pool_Of_Tasks {
	...
	Child_Task *find_task_by_id(int task_id);
	Pool_Status get_pool_status();
	
	//Generic_Pool_Of_Tasks::Generic_Pool_Of_Tasks(void) {
	Generic_Pool_Of_Tasks(void) {
		notify_func = NULL;
		data = NULL;
		num_tasks = 0;
		next_task = 0;
