...
"-a = accumulate/interpolate across missing data\n"
...
// line-of-sight routines for interpolation
int los_start(const unsigned char *image, int xdim, int ydim, int x, int y,
		    int *xout, int *your);
int los_next(int *xout, int *yout);
...
/**
// Weighted interpolation of nearest non-empty map cells to
// fill in an empty cell.
// Return TRUE if interpolated, FALSE if left it empty.
**/
static int interpolate(int x, int y)
{
	float az, ar, ag, ab;	// accumulators 
	float wsum;		// accumulated weight
	int   px, py;

	// accumulate data for pixels in line-of-sight of this point
	if (los_start(vmap, xres, yres, x, y, &px, &py) == FALSE) {
		// failed, set height as empty, set color to black
		zmap->set_float(empty, x, y, 1);
		rgbmap->set_color(x, y, 0, 0, 0);
		return FALSE;
	}

	// weight by inverse distance squared
	az = ar = ag = ab = wsum = 0.0f;
	do {
		float d = 1.0f / ((x-px)*(x-px) + (y-py)*(y-py));
		d *= d;	// ** try distance^4
		az += d * zmap->get_float(px, py);

		uchar r, g, b;
		rgbmap->get_color(px, py, &r, &g, &b);
		ar += d * r;
		ag += d * g;
		ab += d * b;
		wsum += d;
	} while (los_next(&px, &py));

	// store accumulated results
	zmap->set_float(az / wsum, x, y, 1);
	rgbmap->set_color(x, y, int(ar/wsum), int(ag/wsum), int(ab/wsum));
	return TRUE;
}

// Build second band of height map as interpolated version of
// first band
static void interpolate_map()
{
	...
	// copy valid height data
	vmap = new unsigned char[xres*yres];
...
fprintf(stderr, "Row %d\n", y);
	...
	if (verbose)
		fprintf(stderr, "Interpolated %d hole(s)\n", num_interp);
}
